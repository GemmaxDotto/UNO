        ...
        while not shutdown_event.is_set():
            ...
            if received_message.strip().split(";")[1] == "start":
                ...
                if create == 0: #se è il primo giocatore a connettersi
                    ...
                    mazzo_temp.remove(cardCenter) #rimuovo carta centrale da5l mazzo temporaneo5
                    global numeroTurno
                ...
                if giocatoreCorrente == giocatoreTemp: 
                    ...
                    if speciale and correct: #se carta speciale e corretta
                        ...
                        if len(carte)>0: #se carte non vuoto
                            ...
                            conferma_message ="carte_pescate" +  ";" + giocatoreSuccessivo.get_nick() + ";" +carteStr+"\r\n" 
                            print(f"Invio: {conferma_message}")
                            msg.send_messages(giocatoreSuccessivo.get_nick(),conferma_message,giocatori)
                            spostaTurno(1,cambio_verso= False)
                        getGiocatoreCorrenteClient()
                        ...
                        received_messageParts = received_message.strip().split(";")
                        if(giocatoreClient.get_numero_carte()==1 and len(received_messageParts)<3):
                            cardPescate = ""
                            for i in range(0,3):
            ...
            print(f"Numero turno: {numeroTurno}")
            print(f"Verso turno: {verso_turno}")
...
def HandleN_CarteAvversari(nickClient):  
    ...
    """
    giocatore_tmp,pos=searchClient(nickClient)
    for numero in range(clients):
        if not giocatori[numero].get_nick()==nickClient:
             MessagexGUI="CarteAvversari;AVV_"+nickClient+";"+str(giocatore_tmp.get_numero_carte())+"\r\n"
...
def getGiocatoreCorrenteClient():
    for numero in range(clients):
        MessagexGUI = "Turno;"+getGiocatoreCorrente().get_nick() + "\r\n"
        msg.send_messages(giocatori[numero].get_nick(),MessagexGUI,giocatori)
        print(f"Invio: {MessagexGUI}")
...
def gestisciSpeciale(card): 
    ...
    if card[0] == "R":
        ...
        nickGiocatore = getGiocatoreCorrente().get_nick()
        msg = "speciale;" + nickGiocatore + ";pesca_quattro"
        pesca = 4
    elif card == "CC":
...
def checkIsValid(card_lasciata): 
    ...
    """
    speciale = checkCartaSpeciale(getCardComplete(card_lasciata))
    correct = False
    ...
    if speciale and card_lasciata[0]=="T" or card_lasciata[0]=="S" or card_lasciata[0]=="R":
        color_card_speciale = card_lasciata[1]
        if not color_card_speciale == last_color:
            correct = False
        elif color_card_speciale == ColorC:
            correct = True
            ColorC=""
        else: correct = True
...
def checkCartaSpeciale(card):
    ...
    """
    if card=="Draw Four" or card=="Change Color" or card[0]=="R" or card[0]=="S" or card[0]=="D": #carte speciali DRAW FOUR (Draw Four) , CHANGE COLOR (Change Color) , REVERSE (R*) , SKIP (S*) , DRAW TWO (D(raw Two))
        return True
    return False
